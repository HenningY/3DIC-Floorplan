...
exports.parseAnalyticalIterFile = parseAnalyticalIterFile;
...
function parseBlockFile(text) {
    ...
    const outlineRe = /Outline:\s*([-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)\s+([-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)/i;
    const outlines = [];
 ...
 */
function parseFloorplanOutput(text) {
    const lines = text.split(/\r?\n/);
    let idx = 5;
    const modules = [];
    const tsvs = [];
    ...
    while (idx < lines.length) {
        ...
        const m = line.match(/^(\S+)\s+tier(\d+)-(\d+)\s+([0-9.+-eE]+)\s+([0-9.+-eE]+)/);
        if (m) {
            tsvs.push({
                netName: m[1],

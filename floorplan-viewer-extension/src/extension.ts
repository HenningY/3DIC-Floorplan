...
function getHtml(
  ...
  const sceneUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "media", "scene.js")
  );
  const csp = [
    "default-src 'none';",
...
export function activate(context: vscode.ExtensionContext): void {
  const disposable = vscode.commands.registerCommand(
    ...
    () => {
      ...
      const tsvSize =
        cfgFlat.get<number>("floorplanViewer.tsvSize") ?? 3;
      ...
      async function loadAndSend3D(
        ...
        try {
          ...
          appendLog(
            `3D: ${floor.modules.length} modules, ${floor.tsvs.length} TSV markers.`
          );
        } catch (e) {
      ...
      panel.webview.onDidReceiveMessage(
        async (msg) => {
          if (msg.type === "ready") {
            ...
            let defaultOut = "";
            if (root) {
              const guessBlock = path.join(
                root,
                "PA2_3DIC",
                "input_pa2",
                "n100.block"
              );
              const guessNets = path.join(
                root,
                "PA2_3DIC",
                "input_pa2",
                "n100.nets"
              );
              const guessOut = path.join(
                root,
                "PA2_3DIC",
                "analytical",
                "output",
          ...
          if (msg.type === "run") {
            ...
            const cmd = `./analytical ${quotePath(blockFs)} ${quotePath(
              netsFs
            )} ${quotePath(outAbs)}`;
            appendLog(`$ ${cmd}`);
            term.show();

 ...
 */
function resolvePa2ProjectRoot(
    ...
    () => {
      ...
      async function loadAndSend2D(
        ...
        try {
          ...
          appendLog(
            `載入完成：${floor.modules.length} modules、` +
      ...
      panel.webview.onDidReceiveMessage(
        async (msg) => {
          ...
          if (msg.type === "ready") {
            ...
            const analyticalDir = resolveAnalyticalDir(
              cfg,
          ...
          if (msg.type === "deleteConstraint") {
            ...
            try {
              ...
              const targetIdx: number = msg.index;
              if (targetIdx < 0 || targetIdx >= nonEmpty.length) { return; }
          ...
          if (msg.type === "updateConstraint") {
            ...
            try {
              ...
              const newLines = allLines.map((l) => {
                if (!replaced && l === oldLine) { replaced = true; return msg.line as string; }
                return l;
              });
              fs.writeFileSync(constraintFilePath, newLines.join("\n"));
